...
const { viewAllRoles } = require('./roles');
  ...
  // Get all employees
    ...
    const managerId = parseInt(manager.split(':')[0])
      ...
      db.query(sql, params, (err, result) => {
        if (err) {
          ...
          console.log(`Success! ${firstname} ${lastname} has been added to the database.`)
          resolve(result)
        })
      })
    })
    ...
    //updating the employee role and manager
    const updateEmployee = async () => {
      ...
      await inquirer.prompt([
        {
            type: 'list',
            name: 'employee',
            message: 'Select the employee\'s you wish to update:',
      ...
      db.query(sql, params, (err, result) => {
        if (err) {
          ...
          console.log(`Success! The database has been updated.`)
          resolve(result)
        })
      })
    })
